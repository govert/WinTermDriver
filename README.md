...
- JSON `capture` and `wait` responses include the same managed `processHealth` context, and `wtd action <target> restart-session` restarts a managed pane session for agent recovery workflows.
